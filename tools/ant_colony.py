...
class AntColony:
    ...
    def add_sets(self, problem):
        ...
        for i in range(routes):
            for j in range(len(self.problem[i])):
                ...
                self.heuristic[i][j] = 1/score_tree(self.problem[i][j], 'num_children_noreactions')

        self.heuristic = self.heuristic / np.nansum(self.heuristic, axis=1).reshape((-1,1))
        # The heuristic for transition from tree i to tree j is always the
        # same. This code extends the probability along a 3rd axis to make
        # later code faster - since I have to multiply pheromones and heuristic
        # probabilities, it's easier if both matrices have the same dimensions.
        # (this doesn't happen here normally because the heuristics at step i
        # can be expressed with a 1-dimensional matrix, while the pheromones
        # are a 2-dimensional matrix from each node in step i to every node in
        # step i+1).
        self.heuristic = np.tile(self.heuristic[:, np.newaxis, :], (1, max_trees, 1))
        self.pheromones = np.full((routes, max_trees, max_trees), 1/max_trees)
        for i in range(routes):
            r = self.problem[i]
            if len(r) < max_trees:
                ...
                for j in range(max_trees):
                    self.pheromones[i][j] = [1/len(r)]*len(r) + [0]*(max_trees-len(r))
    ...
    def eval_single_path(shared_problem, cumulative_probs, num_ants=50):
        ...
        for i in range(num_ants):
            ...
            for j in range(trees):
                ...
                # cumulative_probs[j][1] means "what are the transition
                # probabilities if I am standing in the 1st node at the j-th
                # step?
                ...
                # table.
                node_probs = cumulative_probs[j][prev_node]
                ...
                # I now remember for the next step which node I selected here.
                prev_node = next_node
            ...
            # It also scores a path better than the path an ant actually goes and therefore implies a wrong success of ACO
    ...
    def run(self, num_steps):
        ...
        for s in range(num_steps):
            ...
            pheromone_probs = self.pheromones / np.nansum(self.pheromones, axis=2)[:, :, None]
            ...
            final_ant_probs = np.cumsum(final_ant_probs, axis = 2)
            ...
            with np.errstate(divide='ignore'):
                a = np.cumsum(final_ant_probs, axis=2)
                b = np.nansum(final_ant_probs, axis=2)[:, :, None]
            ...
            for worker_results in answer_set:
                for cost, trees in worker_results:
                    ...
                    for i in range(len(trees)):
                        # visits[i, last_node, trees[i]] += 1 / 1 + max(((cost-best_cost)/cost), -0.99)
                        if i == 0:  
                            visits[i, 0, trees[i]] += q * len(self.problem) / cost
                        else:
                            visits[i, trees[i-1], trees[i]] += q / (cost / len(self.problem))
...
if __name__ == '__main__':
    ...
    for component_id, component in enumerate(all_components):
        ...
        run_ants = True
        aco = AntColony(workers=10)
        aco.add_sets(smaller_problem)
        if run_ants:
            path = aco.run(num_steps=5)
        else:
        ...
        # Here we can't simply get the old pheromones, besause they depned on the previous node they went.
        # aco.postopt_add(final_routes_top_n_order)
        # smaller_path = aco.run(num_steps = 10)
