...
class AntColony:
    ...
    def run(self,timelimit = 100):
        ...
        x = LpVariable.dicts("molecule_uses_route", (list(range(self.n)), list(range(self.k))), lowBound=0, upBound=1, cat='Binary')
        # Create the problem
        ...
        # Second constraint: forces all buyables to be used in their respective route and molecule
            routes_buyables = {x:[] for x in buyables_target[d]}
...
if __name__ == '__main__':
    ...
    for component_id, component in enumerate(all_components):
        ...
        if run_ants:
            ...
            path = aco.run(timelimit=timelimit)
        else:
