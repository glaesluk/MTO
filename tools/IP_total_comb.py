...
class AntColony:
    ...
    def add_sets(self, trees):
        ...
        print("loaded problem", self.n, self.m)
    ...
    def run_IP(self,startsol = None, timelimit = 100, num_mol_smallTree=25):
        ...
        x = LpVariable.dicts("molecule_uses_route", (list(range(self.n)), list(range(self.m))), lowBound=0, upBound=1, cat=LpInteger)
        # Create a binary variable that indacate whether a target is selected
        z = LpVariable.dicts("target", list(range(self.n)), lowBound=0, upBound=1, cat=LpInteger)
        # Create the problem
        ...
        for d in range(self.n):
            model += (lpSum([x[d][r] for r in range(self.m)]) == z[d], "Molecule_{}_uses_{}_recipe".format(d,z[d]))
            model += (lpSum([x[d][r] for r in range(self.m)]) <= 1, "Molecule_{}_uses_one_recipe".format(d))
        ...
        model += (lpSum(z[d] for d in range(self.n))== num_mol_smallTree, "forces_certain_number_of_recipes")
        model += (lpSum(lpSum([x[d][r] for r in range(self.m)]) for d in range(self.n)) == num_mol_smallTree, "forces_certain_number_of_targets") 
    ...
    def simAnn(self, numMolSmallTree = 25, timelimit = 200):        
        annealing = SimAnn(self.costFunc, self.problem,self.greedy(25))
        print(annealing.best_state)
        return annealing.best_state

    def costFunc(self,path):
        '''
        path: dict{used target:selected route for that target}
        '''
        intermediates = set()
    ...
    def heuristic(self):
        '''
        Gives a heuristic solution for all targets
        '''
        smaller_problem = self.trees
        path = {}
    ...
    def greedy(self, numMol):
        ...
        path = {}
        seen_interm = set()
        for id, molecule in enumerate(self.trees):
            Union = namedtuple('union',['route_id','len_union','union_total'])
            shortest = Union(None,100000, set())
            for route_id, route in enumerate(molecule):
                ...
                if  len(union_now) < shortest.len_union:
                    shortest = Union(route_id, len(union_now), union_now)
            seen_interm.update(shortest.union_total)
            path.update({id:shortest.route_id})
        print(path)
        print("Greedy:", shortest.len_union)
        # This still is pretty random
        sol = {}
...
if __name__ == '__main__':
    ...
    for component_id, component in enumerate(all_components):
        ...
        if run_ants:
            ...
            #path = aco.run_IP(startsol=aco.heuristic(), num_mol_smallTree=num_mol_smallTree, timelimit=timelimit)
            path = aco.simAnn()
            print(path)

            final_routes = []
            for route in path.keys():
                final_routes.append(smaller_problem[route][path[route]])
        else:
        ...
        print("Score:",final_routes_score)
        ...
        if final_routes_test_score < final_routes_score:
            print(final_routes_score, "-->", final_routes_test_score)
        ...
        solutions.append(score_tree({'smiles':'START>>TARGETS', 'children': top_n_routes}, 'num_unique_interm'))
        print(solutions)
