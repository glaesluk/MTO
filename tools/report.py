...
# Use the downloaded utils package
...
def plot_routes(all_routes, outfile=None):
    ...
    graph.attr(overlap="false", splines="true", nodesep = "1", ranksep = "1.5")

    # don't need this one any more
    # graph.node("root", label="Start", fillcolor='beige')
    graph.attr('node', fontname="Arial")
    graph.attr('edge', color="mediumaquamarine", penwidth="5")
    imgdir = "\\".join(outfile.split("\\")[:-1]+ ["img"])
    ...
    for smiles in smiles2id.keys():
        ...
        if smiles in targets:
            ...
            graph.node(str(id), id = str(target_idx), label='', href="{}.html".format(target_idx), target='_top', tooltip = "Target {} \n".format(target_idx)+smiles,
                       xlabel=str(target_idx), shape="box", image ='{}.png'.format(os.path.join(imgdir, smiles_to_filename(smiles))),
                       peripheries="3", style="filled", fillcolor="moccasin", fontsize="60",
                       color="#00293c", penwidth="3")
        elif smiles in buyables:
            # Buyable
            graph.node(str(id),label='', shape='diamond', image ='{}.png'.format(os.path.join(imgdir, smiles_to_filename(smiles))), tooltip=smiles,
                       # style="filled", fillcolor="gray25", color="gray25", fontcolor="white", shape='diamond'
                       style="filled", fillcolor="gray90",peripheries="2", penwidth="3")
        elif '>>' in smiles:
            ...
            # Molecule
            graph.node(str(id), label='',  image ='{}.png'.format(os.path.join(imgdir, smiles_to_filename(smiles))), tooltip=smiles,
                       )
    # Connect all products to the root node - We don't want this any more, because it creates to many (unnecessary) edges
...
def plot_routes_order(synth_order, outfile):
    ...
    Parameters
    ----------
    synth_order : list(set, set, set)
        Set of targets with set of new intermediates and set of buyables (since the previous step)
    ...
    for mol in synth_order:
        mols_acum += len(mol[0])
        all_acum += len(mol[1]) + len(mol[2])
        buyables_acum += len(mol[2])
        iterm_acum += len(mol[1])
